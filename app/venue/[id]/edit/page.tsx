"use client";
import { ChevronLeft, DeleteIcon, PlusCircle, TrashIcon, Upload } from "lucide-react";
...
export default function Page({ params }: { params: { id: number } }) {
  ...
  useEffect(() => {
    const get = async () => {
      ...
      if ((res as ErrorResponse).error) {
        ...
        const imageList = (res as VenueItemProp).images;
        imageList.forEach((img) => {
          setImagesUrl((prevImagesUrl) => [...prevImagesUrl, img.url]);
        });
      }
    };
    //Populating the venue and images.
    get();
  }, []);
  ...
  const imageHandler = () => {
    if (imagesUrl.length >= 5) {
      toast.warning("Limit reached", {
        description: "Only upto 5 images are allowed. For uploading new images, delete the previous images.",
      });
      return;
    }
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    //Checking if the already "uploaded" images and files are upto 5 or less
    if (imagesUrl.length + files.length > 5) {
      toast.warning("Limit reached", {
  ...
  const submitHandler = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    addVenue(newVenue)
      .then(async (res) => {
        ...
        setIsLoading(false);
        toast.success("Editing Successful ✅", { description: "Venue has been edited" });
      })
      .catch((error: Error) => {
                      ...
                      <div className="grid gap-6 sm:grid-cols-3">
                        <div className="grid gap-3">
                          <Select
                            ...
                            defaultSelectedKeys={[venue.venueType]}
                            className="max-w-xs"
                          >
                ...
                <div className="grid auto-rows-max items-start gap-4 lg:gap-8">
                      ...
                      <div className="grid gap-6">
                        <div className="grid gap-3">
                          <Select
                            ...
                            size="md"
                            label="Status"
                            defaultSelectedKeys={[venue.status]}
                            labelPlacement="outside"
                  ...
                  <Card className="overflow-hidden">
                    <CardHeader className="flex justify-between">
                      ...
                      <Button onPress={deleteImagesHandler} variant="flat" size="sm">
                        Delete images
                      </Button>
                    </CardHeader>
                    <CardBody>
                  ...
                  <Card>
                    <CardHeader>
                      <h2 className="font-medium">Delete Venue</h2>
                    </CardHeader>
                    <CardBody>
                      <div className="mb-4">Delete the venue permanently</div>
                      <Button variant="bordered" color="danger" className="font-bold" fullWidth={false} size="sm">
                        Delete
                      </Button>
                    </CardBody>
                  </Card>
